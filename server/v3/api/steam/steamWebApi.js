...
import getHiddenDescriptionFromCacheOrRemote from "./3rdparty/trueachievements.com.js";
import getDataFromSteamStoreFallbackStoreAPI from "./steamStore.js";
import getAchievementList from "./scraper.js";
...
async function generateAchievementList(appID,lang = "english"){
  ...
  let data = {
    current : await getAchievementList(appID),
    previous : null
  };
      
  if (!data) throw "ENOACHIEVEMENT";
  ...
  {  
    try{
      ...
      count.previous = +data.previous.achievement.count;
    }catch{
       data.previous = null;
       count.previous = 0;
    }
   ...
   if (!data.previous || count.current > count.previous) 
   {
     
     console.log("> generating from remote");
     
     let achievements = data.current;
     ...
     if (hiddenCount > 0) 
     {
       ...
       try{
         achievementstatsAPI = await getHiddenDescriptionFromCacheOrRemote(appID, data.current.gameName);
       }catch(err){
         console.warn(err);
     ...
     for(let i=0; i < achievements.length; i++)
     {
      ...
      result = { total: achievements.length, version: count.current, list: achievements };  
    } 
    else 
    {
